...
public class TbModelNumberController {
    ...
    public @ResponseBody
    BaseBean findTbManagerList(HttpSession session,Integer page, Integer size,String brandName, String model, Integer modelType) throws Exception {
        BaseBean bean = new BaseBean();
        try {
            TbManager companyId =  (TbManager) session.getAttribute("dcoinManagerInfo");
            Long com=companyId.getCompanyId();
            List<HashMap> list=tbModelNumberService.onPriceModel(page,size,brandName,model,modelType,com);
            bean.setStatus(200);
            bean.setRows(list);
            bean.setTotal(new PageInfo<>(list).getTotal());
    ...
    public BaseBean add(TbModelNumber tbModelNumber,HttpSession session){
        ...
        try{
            Integer ifmodel = tbModelNumberService.ifmodel(tbModelNumber.getModel());
            if (ifmodel>0){
                baseBean.setMsg("该型号已存在");
                return baseBean;
            }
            TbManager companyId =  (TbManager) session.getAttribute("dcoinManagerInfo");
    ...
    public BaseBean delTbBrand(Long[] delArr) {
        ...
        try {
            for (Long id:delArr){
                Integer ifdeltv = tbModelNumberService.deltv(id);
                if (ifdeltv>0){
                    baseBean.setMsg("请先删除该型号的车电");
                    return baseBean;
                }
            }
