...
public class BranchIndexServiceImpl implements BranchIndexService {
    ...
    @Override
    public List<User> withdrawRental(Map map, Integer page, Integer size) {
        PageHelper.startPage(page, size);
        List<User> users = userMapper.findWnagQiUserId(map);
        for (User user : users) {
            map.put("userId",user.getUserId());
            long wangUserSurplus = userMapper.findWangUserSurplus(map);
            user.setSurplus(wangUserSurplus);
        }
        return users;
    }
