...
public class TbVehicleController {
    ...
    public @ResponseBody
    BaseBean findTbVehicleList(Integer page, Integer size,Long brandId,Integer modelType,String branchName,Long vehicleId,Integer state,String start,String end,String ks,String js) throws Exception {
        BaseBean bean = new BaseBean();
        try {
            List<TbVehicle> list=tbVehicleService.findTbVehicleList(page,size,brandId,modelType,branchName,vehicleId,state,start,end,ks,js);
            bean.setStatus(200);
            bean.setRows(list);
    ...
    public BaseBean okTbVehicleList(HttpSession session,Integer page, Integer size,Integer brandId,Integer modelType,String branchName,String state,Integer tName,String start,String end,String ks,String js){
        TbManager companyId =  (TbManager) session.getAttribute("dcoinManagerInfo");
        Long com=companyId.getCompanyId();
        BaseBean baseBean=new BaseBean();
        try {
            List<HashMap> list = tbVehicleService.okTbVehicleList(page, size, com, brandId, modelType,branchName,state, tName,start,end,ks,js);
            baseBean.setRows(list);
            baseBean.setTotal(new PageInfo<>(list).getTotal());
            baseBean.setStatus(200);
        }catch (Exception e){
    ...
    public BaseBean findRes(@RequestParam(defaultValue ="1",name="page",required = false) Integer page, @RequestParam(defaultValue = "10",name = "size",required = false) Integer size){
        BaseBean baseBean=new BaseBean();
        try {
            List<TbInvoice> res = tbVehicleService.findRes(page, size);
    ...
    public BaseBean findImportUser(
        ...
        try {
            ...
            for (int rowNum = 1; rowNum <= lastRowNum; rowNum++) {
                Integer ifvin = tbVehicleService.ifvin(getCellValue
                        (sheet.getRow(rowNum).getCell(1)));
                if (ifvin>0){
                    bean.setMsg("车架号已存在！");
                    return bean;
                }
                Integer ifgps = tbVehicleService.ifgps(getCellValue
                        (sheet.getRow(rowNum).getCell(2)));
                if (ifgps>0){
                    bean.setMsg("GPS号已存在！");
                    return bean;
                }
                Integer ifserial = tbVehicleService.ifserial(getCellValue
                ...
                veh.setImei(getCellValue(sheet.getRow(rowNum).getCell(2)));
